...
static void _sleep_response_timeout(modbus_t *ctx)
{
	...
	//delay_ms(ctx->response_timeout.tv_sec * 1000);
	delay_us(ctx->response_timeout.tv_usec);
...
int modbus_reply(modbus_t *ctx, const uint8_t *req, int req_length, modbus_mapping_t *mb_mapping){
		...
		case MODBUS_FC_WRITE_SINGLE_COIL: {
			...
			if (mapping_address < 0 || mapping_address >= mb_mapping->nb_bits) {
				...
				if (data == (int)0xFF00 || data == 0x0) {
					...
					#if 0
				} //done to please to autoindenter
				#endif
	...
	gpio_set_pin_level(PB17,1);
	delay_us(5);
	gpio_set_pin_level(PB17,0);
...
void poll(modbus_t *ctx, modbus_mapping_t* _mbMapping)
{
	uint8_t request[MODBUS_RTU_MAX_ADU_LENGTH];
	int requestLength = modbus_receive(ctx, request);
	gpio_set_pin_level(PB16,1);
	delay_us(5);
	gpio_set_pin_level(PB16,0);
	if (requestLength > 0) {
		
		modbus_reply(ctx, request, requestLength, _mbMapping);
	}

}

int coilRead(modbus_mapping_t* _mbMapping, int address)
{
...
int discreteInputRead(modbus_mapping_t* _mbMapping, int address)
{
...
long holdingRegisterRead(modbus_mapping_t* _mbMapping, int address)
{
...
long inputRegisterRead(modbus_mapping_t* _mbMapping, int address)
{
...
int coilWrite(modbus_mapping_t* _mbMapping, int address, uint8_t value)
{
	if (_mbMapping->start_bits > address ||
	(_mbMapping->start_bits + _mbMapping->nb_bits) < (address + 1)) {
		errno = EMBXILADD;
...
int holdingRegisterWrite(modbus_mapping_t* _mbMapping,int address, uint16_t value)
{
	if (_mbMapping->start_registers > address ||
	(_mbMapping->start_registers + _mbMapping->nb_registers) < (address + 1)) {
		errno = EMBXILADD;
