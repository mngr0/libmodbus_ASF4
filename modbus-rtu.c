...
static volatile uint8_t buf_index_r;
static volatile uint8_t buf_index_w;
...
static ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
	...
	size = io_write(io_485,req,req_length);
	return size;
}
...
static int _modbus_rtu_receive(modbus_t *ctx, uint8_t *req)
{
	...
	gpio_set_pin_level(PA19,1);
	delay_us(5);
	gpio_set_pin_level(PA19,0);
...
{
	(void)ctx;
	if((buf_index_w-buf_index_r+BUF_SIZE)%BUF_SIZE < rsp_length){
		return 0;
	}
	CRITICAL_SECTION_ENTER();
	for (int i=0; i<rsp_length; i++)
	{
		rsp[i]=buf[ (buf_index_r +  i) %BUF_SIZE ];
	}
	buf_index_r=(buf_index_r+rsp_length)%BUF_SIZE;
...
{
	//configuration must be done in driver_init.c in ASF4
...
struct timeval *tv, int length_to_read)
{
	...
	//struct usart_sync_descriptor *descr  = CONTAINER_OF(io_485, struct usart_sync_descriptor, io);
	//     unsigned long wait_time_millis = (tv == NULL) ? 0 : (tv->tv_sec * 1000) + (tv->tv_usec / 1000);
	//     unsigned long start = millis();
	//
	uint32_t max = 0x0007FFFF;
	do {
		max--;
		int k =0;
		int tmp =0;
		CRITICAL_SECTION_ENTER();
		k = (buf_index_w-buf_index_r+BUF_SIZE)%BUF_SIZE >= length_to_read;
		tmp= buf_index_w-buf_index_r;
		CRITICAL_SECTION_LEAVE();
		if (k) {
			return tmp;
		}
...
static void rx_cb(const struct usart_async_descriptor *const io_descr)
{
	...
	buf[buf_index_w++]=b;
	buf_index_w=(buf_index_w)%BUF_SIZE;
	CRITICAL_SECTION_LEAVE();
}
...

{
	...
	usart_async_get_io_descriptor(&USART_0, &io_485);
